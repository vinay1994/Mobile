...
public class YourProfileModule extends Base
{
	...
	public void updateClassAndSection(WebDriver driver, String className, String sectionName) throws InterruptedException
	{
		...
		GenericFunctions.mouseHoverAndClick(driver, heaMod.profileImg, heaMod.myProfileLnk);
		
		GenericFunctions.waitForElementVisibility(driver, classEditImg);
		classEditImg.click();
		extentTest.log(Status.PASS, "Click on Class Edit Image Icon.");
		
		GenericFunctions.selectValueFromList(driver, classLst, className);
		extentTest.log(Status.PASS, "Update Class to: "+className);

		GenericFunctions.selectValueFromList(driver, sectionLst, sectionName);
