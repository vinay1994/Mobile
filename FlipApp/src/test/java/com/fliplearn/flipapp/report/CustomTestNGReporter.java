package com.fliplearn.flipapp.report;
...
public class CustomTestNGReporter implements IReporter {
	...
	private String readEmailabelReportTemplate()
	{
		...
		try {
			...
			File file = new File(Constants.EMAILABLE_REPORT_TEMPLATE_FILE);
...
//					retBuf.append("</td>");
					...
					retBuf.append("<td bgcolor='#33FFE0'>");
...
//					retBuf.append("</td>");
	...
	private String convertDeltaTimeToString(long deltaTime)
	{
		...
		//retBuf.append(f.format(hours) + ":" + f.format(minutes) + ":" + f.format(seconds));
